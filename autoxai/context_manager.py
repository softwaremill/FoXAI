...
CV_Explainer = TypeVar('CV_Explainer', bound=CVExplainer)
...
class AutoXaiExplainer():
    ...
    def __init__(self, 
        ...
        self.explainer_map:Dict[Explainers,CV_Explainer] = \
            {explainer_name.name: getattr(explainer,explainer_name.value)() for explainer_name in explainers}
    ...
    def __enter__(self) -> "AutoXaiExplainer":
        ...
        if self.model.training:
            raise ValueError("The autoxai should be run on model, which is in eval mode.")
        return self

    def __exit__(self, exc_type: Any, exc_value: Any, traceback: Any) -> None:
        pass
    
    def __call__(self, *args, **kwargs) -> Tuple[Any, Dict[Explainers,torch.tensor]]:
        ...
        explanations:Dict[Explainers, torch.tensor] = {}
        for explainer_name in self.explainer_map:
            explanations[explainer_name] = self.explainer_map[explainer_name].calculate_features(
                model=self.model,
                input_data=args,
...
if __name__ == "__main__":
    class SampleModel(torch.nn.Module):
        def __init__(self, in_channels:int=1,resolution:int=224,):
            super().__init__()
            self.stride:int = 16
            ...
            output_channels:float = ((resolution//self.stride)**2)*self.out_channels
    ...
    model:torch.nn.Module = SampleModel(in_channels=1).eval()
